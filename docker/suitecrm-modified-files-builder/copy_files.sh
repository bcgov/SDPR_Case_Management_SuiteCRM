...
SOURCE_DIR="./suitecrm-modified-files-builder"
...
copy_files() {
    ...
    if [[ "$src_file" == "$(realpath "$0")" ]] || [[ "$dest_file" == "/suitecrm/public/legacy/config_override.php" ]] || [[ "$dest_file" == "/suitecrm/public/legacy/config.php" ]]; then
        return
    fi
