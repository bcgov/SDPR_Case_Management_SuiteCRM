...
import {
  UserPreferenceMap,
  UserPreferenceStore,
} from "../../../store/user-preference/user-preference.store";
import {
  ScreenSize,
...
export class BaseNavbarComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  userPreferences$: Observable<UserPreferenceMap> =
    this.userPreferenceStore.userPreferences$;
  ...
  recentlyViewed$: Observable<RecentlyViewed[]> =
    this.globalRecentlyViewedStore.globalRecentlyViewed$;
  ...
  ngOnInit(): void {
    ...
    this.recentlyViewedCount = this.systemConfigStore.getUi(
      "global_recently_viewed",
    );

    this.subs.push(
      this.notificationStore.notificationsEnabled$.subscribe(
  ...
  protected calculateMaxTabs(navigation: Navigation): void {
    const sizeMap = this.systemConfigStore.getConfigValue(
  ...
  getModuleQuickActions(module: string): void {
    ...
    this.currentQuickActions = actions.filter(
      (action, index, self) =>
        index ===
        self.findIndex(
          (a) => a.labelKey === action.labelKey && a.module === action.module,
        ),
    );
  }
  ...
  search(searchTerm: string) {
    const searchController =
      this.systemConfigStore.getConfigValue("search")?.controller ?? "";
